 ...
 * @author hansolo
 */
public class AzimuthDial extends AbstractRadial {
...
// Do not create the ship picture        create_SHIP_Image(GAUGE_WIDTH, bImage);
...
// Do not create the 2nd pointer        pointer2Image = create_POINTER_Image(GAUGE_WIDTH, pointer2Type, pointer2Color, customPointer2Color);
    ...
    protected BufferedImage create_TICKMARKS_Image(final int WIDTH, final double FREE_AREA_ANGLE,
        ...
        for (double alpha = (2.0 * Math.PI); alpha >= STEP; alpha -= STEP) {
            ...
            if (counter == 30 || counter == 0) {
                ...
                // Print out the cardinal directions
                if(valueCounter == 0) {
                    G2.fill(UTIL.rotateTextAroundCenter(G2, "N", (int) TEXT_POINT.getX(), (int) TEXT_POINT.getY(), 0));
                } else if(valueCounter == 90) {
                	switch(loc_cnt) {
                	case 0:
                    	G2.fill(UTIL.rotateTextAroundCenter(G2, "W", (int) TEXT_POINT.getX(), (int) TEXT_POINT.getY(), 0));
                		break;
                	case 1:
                        G2.fill(UTIL.rotateTextAroundCenter(G2, "E", (int) TEXT_POINT.getX(), (int) TEXT_POINT.getY(), 0));
                		break;
                	}
                	loc_cnt++;
                } else if(valueCounter == 180) {
                    G2.fill(UTIL.rotateTextAroundCenter(G2, "S", (int) TEXT_POINT.getX(), (int) TEXT_POINT.getY(), 0));
            	} else{
